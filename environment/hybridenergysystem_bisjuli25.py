...
#         return observations, self.episode_reward, terminated, truncated, info
    ...
    def calc_reward(self):
        ...
        # switching costs
        self.state["reward_batteryaging"] = -(
            switch_cost(
                self.state_backup["s_actual_BESS"],
                self.state["s_actual_BESS"],
                (30000/6800),
            ))    

        # energy loss
        self.state['reward_energy_loss'] = - abs(self.state['s_losses_BESS'] 
                                              + self.state['s_losses_KESS']) * 0.3
        #self.state['s_price_electricity']

        #reward compensation
        self.state['energy_through_compensation'] = (self.state['s_actual_BESS'] 
                                                    + self.state['s_actual_KESS']
                                                    - self.state['s_losses_KESS']
                                                    - self.state['s_losses_BESS'])
        self.state['energy_after_compensation'] = (self.state['d_usage']) + self.state['energy_through_compensation']
        #self.state['reward_energy_compensated'] = - abs(self.state['energy_after_compensation'] * self.state['s_price_electricity'])

        #self.state["activation_energy"] = self.state['p_production_delta_electric_power_1h']
        #self.state['reward_energy_compensated'] = - ((self.state['energy_after_compensation'] - self.state['activation_energy'])**2 * self.state['s_price_electricity'])
        #self.state['reward_energy_compensated'] = - ((self.state['energy_after_compensation'] - 24000)**2 * self.state['s_price_electricity'])


        #self.state['energy_through_compensation'] =self.state['d_usage'] - self.state['s_actual_BESS'] - self.state['s_actual_KESS'] - self.state['reward_energy_loss']       

        if  self.state['energy_after_compensation'] > self.state['p_production_delta_electric_power_6h'] :  
        #    self.state['reward_energy_compensated'] =-abs(240000  -  self.state['energy_after_compensation'])**2 * self.state['s_price_electricity']- 4000
        #else:
        #    self.state['reward_energy_compensated'] =int(0)
        
            self.state['reward_energy_compensated'] = - abs((self.state['energy_after_compensation'] 
                                                    + self.state['s_losses_KESS']
                                                    + self.state['s_losses_BESS'])* (0.3+17.72))
        else: 
            self.state['reward_energy_compensated'] = - abs(self.state['d_usage'] 
                                                    + self.state['s_actual_BESS'] 
                                                    + self.state['s_actual_KESS']
                                                    - self.state['s_losses_KESS']
                                                    - self.state['s_losses_BESS'])* (0)
            
            #self.state['s_price_electricity']            
        
        # if  self.state['output'] > self.activation_energy :  
        #     self.state['reward_energy_compensated'] =-abs((self.activation_energy  -  self.state['output'])/1000)**2 - 400
        # else:
        #     self.state['reward_energy_compensated'] =int(0)
        ...
        # if  self.state['output'] > self.activation_energy :  
        #     self.state['reference_energy_compensated'] =-abs(self.activation_energy  -  self.state['output'])
        # else:
        #     self.state['reference_energy_compensated'] =int(0)


        # #reference compensation withoutstorage
        # self.state['output1'] =self.state['d_usage'] 

        # if  self.state['output1'] > self.activation_energy : 
        #     self.state['reward_energy_withoutcompensated'] =-(abs(self.activation_energy  -  self.state['output1']))
        # else:
        #     self.state['reward_energy_withoutcompensated'] =int(0) 

        # #diff
        # self.state['comp_diff'] = self.state['reward_energy_withoutcompensated'] - self.state['reference_energy_compensated']
                
        # # energy costs
        # base_power_electric =  self.state['output'] # total consumption 
        # #self.state['reward_energy_electric'] = -self.state['s_price_electricity']*base_power_electric*self.sampling_time/3600/1000    
        ...
        # total reward
        self.state['reward_total'] =   (self.state['reward_energy_compensated'] + self.state['reward_energy_loss'] + self.state.get('reward_batteryaging', 0)+ self.state["reward_other"])
                                            # self.state['reward_energy_loss'] + \
                                            # self.state['reward_other'] + \
                                            # self.state['reward_energy_electric'] + \
                                            
                                           # self.state['reward_storage_status']
    ...
    def reset(self, *, seed: int | None = None, options: dict[str, Any] | None = None) -> np.ndarray:
        ...
        if self.n_steps > 0 :
            ...
            #self.episode_statistics['rewards_total_norm'] = self.episode_df['reward_total_norm'].sum()
            self.episode_statistics['rewards_batteryaging'] = self.episode_df['reward_batteryaging'].sum()
            self.episode_statistics['rewards_energy_loss'] = self.episode_df['reward_energy_loss'].sum()
            #self.episode_statistics['rewards_energy_electric'] = self.episode_df['reward_energy_electric'].sum()
            self.episode_statistics['rewards_energy_compensated'] = self.episode_df['reward_energy_compensated'].sum()
            self.episode_statistics['rewards_other'] = self.episode_df['reward_other'].sum()
            self.episode_statistics['energy_through_compensation'] = self.episode_df['energy_through_compensation'].sum()/3600
            self.episode_statistics['energy_after_compensation'] = self.episode_df['energy_after_compensation'].sum()/3600
            self.episode_statistics['n_steps'] = self.n_steps
            self.episode_statistics["time"] = time.time() - self.episode_timer
            self.episode_statistics['average_SOC_BESS']    = self.episode_df['s_SOC_BESS'].mean()
            self.episode_statistics['average_SOC_KESS']    = self.episode_df['s_SOC_KESS'].mean()
            self.episode_statistics['charge_cycle_KESS']   = self.episode_df['s_SOC_KESS'].diff().abs().sum()
            self.episode_statistics['charge_cycle_BESS']   = self.episode_df['s_SOC_BESS'].diff().abs().sum()
            self.episode_statistics['compensated']         = self.episode_df['reward_energy_compensated'].sum()  
            self.episode_statistics['max_last']         = self.episode_df['energy_after_compensation'].max()  
    ...
    def render_episodes(self):
        ...
        axes[1].plot(
            x, y["rewards_energy_compensated"], label="Kompensation", color=(0.44, 0.19, 0.63), linewidth=1, alpha=0.9
        )
